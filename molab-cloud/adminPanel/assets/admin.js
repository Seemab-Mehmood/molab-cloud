    ...
    .map(h => `
      <tr>
        ...
        <td style="text-align:right; white-space:nowrap;">
          <button class="btn btn-ghost" style="padding:5px 8px;" onclick="openReviewModal('${h.id}')">Review</button>
...
function openReviewModal(id){
  ...
  document.getElementById('reviewBody').innerHTML = `
    <div><div class="label-sm">Hospital</div><div>${h.name}</div></div>
    <div><div class="label-sm">Type</div><div>${h.type}</div></div>
    <div><div class="label-sm">City</div><div>${h.city}</div></div>
    <div><div class="label-sm">Country</div><div>${h.country}</div></div>
    <div><div class="label-sm">Representative</div><div>${h.repName}</div></div>
    <div><div class="label-sm">Representative email</div><div>${h.repEmail}</div></div>
    <div><div class="label-sm">Representative phone</div><div>${h.repPhone||'—'}</div></div>
    <div><div class="label-sm">Email verified</div><div>${h.emailVerified ? 'Yes' : 'No — representative has not clicked their verification link yet'}</div></div>
    <div><div class="label-sm">Current status</div><div>${statusPill(h.status)}</div></div>
    <div><div class="label-sm">Patients on file</div><div>${h.patientCount}</div></div>
    <div style="grid-column:1/-1;"><div class="label-sm">Submitted on</div><div>${new Date(h.createdAt).toLocaleString()}</div></div>
  `;
  const approveBtn = document.getElementById('reviewApproveBtn');
  const suspendBtn = document.getElementById('reviewSuspendBtn');
  approveBtn.style.display = h.status === 'approved' ? 'none' : 'inline-flex';
  suspendBtn.innerText = h.status === 'suspended' ? 'Reinstate' : 'Suspend';
  approveBtn.onclick = async () => { await setStatus(h.id, 'approved'); closeReviewModal(); };
  suspendBtn.onclick = async () => { await setStatus(h.id, h.status === 'suspended' ? 'approved' : 'suspended'); closeReviewModal(); };
